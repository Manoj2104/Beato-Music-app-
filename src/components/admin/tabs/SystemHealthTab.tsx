    ...
    <div style={{ color: '#221a15', fontFamily: 'Inter, sans-serif' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ ...card(), marginBottom: 24, display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12, color: '#87786c' }}>Last updated: {lastUpdated}s ago</p>
        </div>
        <div style={{ marginLeft: 'auto' }}>
          <button onClick={() => { fetchServices(); toast.success('Status page refreshed'); }} style={{ background: '#f4eede', border: 'none', borderRadius: 8, color: '#a0958b', padding: '7px 14px', fontSize: 12, cursor: 'pointer' }}>↻ Refresh</button>
        </div>
      </motion.div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 24 }}>
        {loading && services.length === 0 ? (
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 20, color: '#87786c' }}>Loading system health...</div>
        ) : (
          services.map(s => (
            <div key={s.name} style={{ ...card(), borderLeft: `3px solid ${statusColors[s.status] || '#888'}`, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 12 }}>
                  <div><p style={{ margin: 0, fontSize: 10, color: '#87786c' }}>Uptime</p><p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: '#b08850' }}>{s.uptime}%</p></div>
                  <div><p style={{ margin: 0, fontSize: 10, color: '#87786c' }}>Response</p><p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: '#221a15' }}>{s.respMs}ms</p></div>
                  <div style={{ gridColumn: '1/-1' }}><p style={{ margin: 0, fontSize: 10, color: '#87786c' }}>Req/min</p><p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: '#a0958b' }}>{(s.rpm || 0).toLocaleString()}</p></div>
                </div>
              </div>

              {/* Status Simulator Controls */}
              <div style={{ borderTop: '1px solid #1a1a1a', paddingTop: 10, marginTop: 'auto' }}>
                <p style={{ margin: '0 0 6px 0', fontSize: 9, color: '#87786c', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>Simulate Status</p>
            ...
            {[
              ...
              <div key={m.label}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                  <span style={{ fontSize: 12, color: '#a0958b' }}>{m.label}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: m.color }}>{m.value}{m.unit}</span>
                </div>
                {m.unit === '%' && (
                  <div style={{ height: 6, background: '#f4eede', borderRadius: 3, overflow: 'hidden' }}>
        ...
        <div style={card()}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={errorData}>
              ...
              <XAxis dataKey="hour" tick={{ fill: '#87786c', fontSize: 9 }} interval={5} />
              <YAxis tick={{ fill: '#87786c', fontSize: 9 }} unit="%" />
              <Tooltip contentStyle={{ background: '#f4eede', border: '1px solid rgba(43,34,26,0.12)', borderRadius: 8, color: '#221a15' }} />
      ...
      <div style={card()}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {incidents.map((inc, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#fbf9f5', borderRadius: 10, padding: '12px 16px', border: `1px solid ${sevColors[inc.severity]}33` }}>
              ...
              <span style={{ fontSize: 11, color: '#87786c' }}>Duration: {inc.duration}</span>
