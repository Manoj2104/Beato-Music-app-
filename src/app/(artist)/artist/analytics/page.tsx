...
function KpiCard({ icon: Icon, label, value, change, positive, color }: any) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      style={{ padding: 22, borderRadius: 16, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 100, background: positive ? 'rgba(29, 185, 84,0.12)' : 'rgba(239,68,68,0.12)', color: positive ? G : '#ef4444' }}>
          {change}
        </span>
      </div>
      <p style={{ fontFamily: 'Outfit, sans-serif', fontSize: 28, fontWeight: 900, color: '#fff', lineHeight: 1 }}>{value}</p>
      <p style={{ color: '#737373', fontSize: 12, marginTop: 5 }}>{label}</p>
    </motion.div>
  );
}
...
export default function ArtistAnalyticsPage() {
  ...
  return (
    <div style={{ minHeight: '100%', background: '#0a0a0a', padding: '24px 28px 60px' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 28, fontWeight: 900, color: '#fff' }}>Analytics</h1>
        </div>
        <div style={{ display: 'flex', gap: 4, background: 'rgba(255,255,255,0.06)', borderRadius: 10, padding: 3 }}>
          {RANGES.map(r => (
            <button key={r} onClick={() => setRange(r)} style={{
              ...
              background: range === r ? G : 'transparent', color: range === r ? '#000' : '#737373', transition: 'all 0.15s',
            }}>{r}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20, marginBottom: 20 }}>
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 18, padding: '22px', border: '1px solid rgba(255,255,255,0.07)' }}>
          <h3 style={{ color: '#fff', fontWeight: 700, fontSize: 15, marginBottom: 18 }}>Streams & Listeners</h3>
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={data}>
              ...
              <YAxis stroke="#525252" fontSize={11} tickFormatter={v => `${(v / 1000).toFixed(0)}K`} />
              <Tooltip contentStyle={{ background: '#1e1e1e', border: '1px solid #333', borderRadius: 8, color: '#fff' }} formatter={(v: any, n) => [`${(Number(v) / 1000).toFixed(1)}K`, n === 'streams' ? 'Streams' : 'Listeners']} />
        ...
        {/* Geo */}
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 18, padding: '22px', border: '1px solid rgba(255,255,255,0.07)' }}>
          <h3 style={{ color: '#fff', fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Top Countries</h3>
          ...
          {geoData.map(c => (
            <div key={c.country} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 7 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ color: '#fff', fontSize: 12, fontWeight: 600, width: 28, textAlign: 'right' }}>{c.pct}%</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 20 }}>
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 18, padding: '22px', border: '1px solid rgba(255,255,255,0.07)' }}>
          <h3 style={{ color: '#fff', fontWeight: 700, fontSize: 15, marginBottom: 18 }}>Audience Age</h3>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={ageData} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" horizontal={false} />
              <XAxis type="number" stroke="#525252" fontSize={10} tickFormatter={v => `${v}%`} />
              <YAxis dataKey="range" type="category" stroke="#525252" fontSize={11} />
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 14, justifyContent: 'center' }}>
            <div style={{ textAlign: 'center' }}>
              <p style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>58%</p>
              <p style={{ color: '#737373', fontSize: 10 }}>Male</p>
            </div>
            <div style={{ width: 1, background: 'rgba(255,255,255,0.1)' }} />
            <div style={{ textAlign: 'center' }}>
              <p style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>39%</p>
              <p style={{ color: '#737373', fontSize: 10 }}>Female</p>
            </div>
            <div style={{ width: 1, background: 'rgba(255,255,255,0.1)' }} />
            <div style={{ textAlign: 'center' }}>
              <p style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>3%</p>
              <p style={{ color: '#737373', fontSize: 10 }}>Other</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 18, border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden' }}>
          <div style={{ padding: '18px 20px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <h3 style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>Top Tracks Performance</h3>
          </div>
          <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ color: '#525252', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            ...
            <tbody>
              {topTracks.map((t, i) => (
                <tr key={t.title} style={{ borderTop: '1px solid rgba(255,255,255,0.05)', transition: 'background 0.15s' }}
                  onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.04)')}
                  onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                  <td style={{ padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ color: '#525252', width: 16 }}>{i + 1}</span>
                    <span style={{ color: '#fff', fontWeight: 500 }}>{t.title}</span>
                  </td>
                  <td style={{ padding: '13px 16px', textAlign: 'right', color: '#a3a3a3' }}>{(t.streams / 1_000_000).toFixed(1)}M</td>
                  <td style={{ padding: '13px 16px', textAlign: 'right', color: '#a3a3a3' }}>{(t.saves / 1000).toFixed(0)}K</td>
                  <td style={{ padding: '13px 16px', textAlign: 'right', color: '#a3a3a3' }}>{t.skipRate}</td>
