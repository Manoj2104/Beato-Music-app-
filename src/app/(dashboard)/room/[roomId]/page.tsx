...
export default function JamRoomPage({ params }: { params: Promise<{ roomId: string }> }) {
  ...
  const fetchRoomInfo = async (joinRoom = false, enteredPassword?: string) => {
    try {
      ...
      if (data.success && data.room) {
        ...
        // Auth issue - don't redirect, just stop loading (user will see login page via middleware)
        console.warn('Auth required for room join');
      } else {
        setIsPlaying(false);
        if (typeof window !== 'undefined') {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`/api/rooms/${roomId}`, { method: 'GET', credentials: 'include' });
        if (res.status === 404) {
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flex: 1 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <p style={{ fontSize: 12.5, fontWeight: 700, margin: 0, color: '#0f172a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{track.title}</p>
                    ...
                    <div style={{
                      ...
                      {allTracks.slice().reverse().map((track) => (
                        ...
                        >
                          ...
                          <p style={{ fontSize: 11, fontWeight: 700, margin: 0, color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {track.title}
                          </p>
                  ...
                  <div style={{ width: '100%', boxSizing: 'border-box', marginTop: 20 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', boxSizing: 'border-box' }}>
                      {allTracks.slice((allTracksPage - 1) * tracksPerPage, allTracksPage * tracksPerPage).map((track, i) => (
                        ...
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flex: 1 }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: '#9ca3af', width: 20, textAlign: 'center', flexShrink: 0 }}>
                            ...
                            <div style={{ minWidth: 0, flex: 1 }}>
                              <p style={{ fontSize: 12.5, fontWeight: 700, margin: 0, color: '#0f172a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {track.title}
                              </p>
