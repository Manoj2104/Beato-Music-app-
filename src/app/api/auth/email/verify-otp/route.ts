...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Fetch OTP record
    const otp = db.getOtp(emailTrimmed);
    if (!otp) {
      return NextResponse.json({ error: 'No active OTP verification session found for this email' }, { status: 400 });
    }

    // 2. Increment attempts and validate threshold (max 3)
    const attempts = db.incrementOtpAttempts(emailTrimmed);
    if (attempts > 3) {
      db.deleteOtp(emailTrimmed);
