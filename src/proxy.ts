...
const ADMIN_ROLES = ['ADMIN', 'SUPER_ADMIN', 'admin', 'super_admin', 'moderator', 'analyst', 'MODERATOR', 'ANALYST'];
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    if (pathname.startsWith('/api/upload/') || pathname === '/api/upload-song') {
      ...
      if (userRole !== 'ARTIST' && userRole !== 'artist' && (!userRole || !ADMIN_ROLES.includes(userRole))) {
        return addCorsHeaders(request, NextResponse.json({ error: 'Forbidden: Insufficient privileges' }, { status: 403 }));
      }
    }
